...
#Load home-made neural network
model = load_model("Neural Networks/PT_3_V1.h5")
...
#[INFO]: adaptive thresholding, doesnt work properly yet => binary threshold too low for OCR
#img_blur = cv2.medianBlur(image, 5)
#im_th = cv2.adaptiveThreshold(image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 11, 2)
...
#Contour recognition and sorting
...
#Contour selection (Threshholds, size), resizing of the selected area (padding) and prediction with neural network => looped for every contour in 'cnts'
for c in cnts:
	...
	if (w >= wMin and w < wMax) and (h >= hMin and h <= hMax):
		...
		if tW > tH:
			...
			#plt.imshow(thresh)
			#plt.show()
			#thresh = imutils.resize(thresh, width=int(tW*32/tH))
